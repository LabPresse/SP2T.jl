...
active = ones(Bool, size(frames, 3), 1, 2)
active[1:255, :] .= false
active[end-254:end, :] .= false
h = Brightness{FloatType}(
    scalarvalue = 1e4 * metadata["period"],
    active = CuArray(active),
    prior = Gamma(1, 1),
    proposalparam = 1,
)
...
runMCMC!(chain, x, CuArray(active), M, msd, h, CuArray(frames), detector, psf, 50_000, true);

jldsave("./example/2D/chain_gpu_fixed.jld2"; chain)
